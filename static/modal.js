...
const modalModule = {
     ...
     */
    addEventListeners() {
        // 使用事件委托为所有详情按钮添加点击事件监听器
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('details-btn')) {
                let matchId = e.target.getAttribute('data-fid');
                ...
                const matchTime = e.target.getAttribute('data-match-time');
                const homeTeam = e.target.getAttribute('data-home-team');
                const awayTeam = e.target.getAttribute('data-away-team');
                const homeTeamLogo = e.target.getAttribute('data-home-team-logo');
                const awayTeamLogo = e.target.getAttribute('data-away-team-logo');
                ...
                // 增强的fid获取逻辑：
                // 1. 首先尝试从data-fid获取matchId
                // 2. 如果data-fid为空，从tr元素的id属性提取（格式为a+数字）
                if (!matchId && parent) {
                    // 检查tr元素是否有id属性且以a开头
                    if (parent.id && parent.id.startsWith('a')) {
                        matchId = parent.id.substring(1);
                    }
                }
                
                // 增强的sid获取逻辑：
                // 1. 首先尝试从data-sid获取sid
                // 2. 如果data-sid为空，尝试从联赛链接提取
                if (!sid && parent) {
                    const leagueLink = parent.querySelector('.ssbox_01 a');
                    if (leagueLink) {
                        const leagueHref = leagueLink.getAttribute('href');
                        if (leagueHref && leagueHref.includes('zuqiu-')) {
                            const sidMatch = leagueHref.match(/zuqiu-(\d+)/);
