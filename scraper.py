...
class MatchScraper:
    ...
    def fetch_live_matches(cls, date=None):
        ...
        try:
            ...
            # 3. 根据是否传入日期和比赛类型选择不同的URL
            if date:
                if is_future_match:
                    # 未来比赛使用weekfixture.php
                    url = f"https://live.500.com/weekfixture.php?e={date}"
                else:
                    # 历史比赛使用wanchang.php
            ...
            for idx, row in enumerate(match_rows):
                try:
                    ...
                    # 对于未来比赛，从tr的id属性中提取fid（格式为a+数字）
                    tr_id = row.get("id", "")
                    if tr_id.startswith("a"):
                        fid_from_id = tr_id[1:]
                        # 优先使用从id提取的fid，覆盖row.get("fid")的结果
                        fid = fid_from_id
                        logger.info(f"从tr id提取到fid: {fid}, 原始fid: {row.get('fid', '')}")
                    
                    # 如果tr标签没有fid，尝试从比分链接中获取
                    if not fid:
                        # 查找比分相关的链接
                        score_links = row.find_all("a")
                        for link in score_links:
                            href = link.get("href", "")
                            if "detail.php?fid=" in href:
                                fid_match = re.search(r"fid=(\d+)", href)
                                if fid_match:
                                    fid = fid_match.group(1)
                                    break
                    
                    # 如果tr标签没有sid，尝试从联赛链接中获取
                    if not sid:
                        if league:
                            league_href = league.get("href", "")
                            sid_match = re.search(r"zuqiu-(\d+)", league_href)
                            if sid_match:
                                sid = sid_match.group(1)
                    
                    # 对于未来比赛，如果没有sid，尝试从lid属性获取
                    if not sid:
                        lid = row.get("lid", "")
                        if lid:
                            sid = lid
...
class OddsScraper:
    ...
    def fetch_yapan_data(match_id):
        ...
        if not res:
            logger.warning(f"获取亚盘数据请求失败，比赛ID: {match_id}")
            return None
        ...
        try:
            ...
            # 方式2: 如果方式1失败，尝试通过class查找
            if not data_table:
                data_table = soup.find("table", class_="datatb")
            
            # 方式3: 如果方式2失败，尝试查找所有表格并筛选包含公司数据的表格
            if not data_table:
                all_tables = soup.find_all("table")
                for table in all_tables:
                    # 查找包含公司名称的行（id为数字的tr标签）
                    company_rows = table.find_all("tr", id=re.compile(r"^\d+$")
									   )
                    if company_rows:
                        data_table = table
                        break
            ...
            if not data_table:
                logger.warning(f"未找到亚盘数据表格，比赛ID: {match_id}")
                return None
            ...
            if not company_rows:
                logger.warning(f"未找到亚盘公司数据行，比赛ID: {match_id}")
                return None
            ...
            for row in company_rows:
                try:
                    ...
                    logger.debug(f"解析亚盘公司数据行失败: {e}")
                    continue
            
            if not extracted_data:
                logger.warning(f"未解析到有效亚盘数据，比赛ID: {match_id}")

            return extracted_data
        except Exception as e:
            logger.error(f"解析亚盘数据失败，比赛ID: {match_id}, 错误: {e}")
            logger.debug(f"响应内容: {res.text[:1000]}...")  # 记录部分响应内容用于调试
            return None
